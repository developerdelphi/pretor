        ...
        <v-card>
          ...
          <v-card-text>
            <v-container grid-list-md>
              <v-layout wrap>
                <v-flex xs12 sm6 md4>
                  <v-text-field v-model="table.name" label="Proceso"></v-text-field>
                </v-flex>
                <v-flex xs12 sm6 md4>
                  <v-text-field v-model="table.origin" label="Entidade"></v-text-field>
                </v-flex>
                <v-flex xs12 sm6 md4>
                  <v-text-field v-model="table.origin" label="Partes"></v-text-field>
                </v-flex>
              </v-layout>
            </v-container>
          </v-card-text>
      ...
      >
        ...
        <template v-slot:items="props">
          <tr @click="props.expanded = !props.expanded">
            ...
            <td class="text-xs-right">{{ props.item.entity.name }}</td>
            <td class="text-xs-right">{{ props.item.stakeholder.persona.name }}</td>
            <td class="justify-center layout right px-0">
              ...
              <v-btn
                ...
                dark
                small
                @click="deleteData(props.item.id, 'process')"
                color="red darken-4"
              >
...
export default {
  ...
  data() {
    return {
      ...
      status: null,
      headers: [
        { text: "Processo", align: "center", sortable: true, value: "name" },
        { text: "Entidade", align: "center", value: "entity.name" },
        { text: "Áreas", align: "center", value: "area.name" },
        { text: "Partes", align: "center", value: "stakeholder.persona.name" }
      ]
    };
  },
  mounted() {
